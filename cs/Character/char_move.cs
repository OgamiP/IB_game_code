...
//	public ParticleSystemRenderer Jump_particle;
	...
	public int jump_controller = 0;//ジャンプボタンが高速に連続で押されることを防ぐ
	public int rolling_duration_flag = 0;//回避行動中かどうか判定する
	public float rolling_duration = 5f;

	public float move_speed = 0.2f;//通常移動速度
	...
	void Start () {
		...
		CHAR_SOUND = GetComponent<char_sound> (); 
...
//			Jump_particle.enabled = false;
	...
	//ジャンプボタンを短い時間で連続して押されないよう制御する
	public IEnumerator Jump_Wait(float duration_time)
	{
		//ジャンプ制御変数を1にする
		jump_controller = 1;
		yield return new WaitForSeconds (duration_time);
		//途中でキャラクターが接地したら待ちキャンセル
		if (jumpcount == 0) {
			yield break;
		}
	...
	void Update () {
		...
		if (jumpcount == 0 && Input.GetButtonDown ("Jump") && SKILL_FLAG.Skill_Flag == 0 && rolling_duration_flag == 0 && Gun_Mode == 0) {
			GetComponent<char_sound>().landing_sound_flag = 0;//着地サウンドフラグ0に
			...
			RG.AddForce (transform.up * 50f, ForceMode.Impulse);
			anim.SetBool (GotoJump1ID, true);
			//高速にジャンプボタンを連打しても一定時間で3段ジャンプ目に遷移するため待つ
			StartCoroutine("Jump_Wait",0.5f);

		} else if (jump_controller == 0 && jumpcount == 1 && Input.GetButtonDown ("Jump")) {//2段ジャンプ
			jumpcount++;
...
//			Jump_particle.enabled = true;
		...
		} else if (jump_controller == 0 && jumpcount == 2 && Input.GetButtonDown ("Jump")) {//3段ジャンプ
			jumpcount++;
		...
		} else if (jump_controller == 0 && jumpcount >= 3 && Input.GetButtonDown ("Jump") ) {//飛行
			...
			{			//jumpcountは4以上を保つ
				...
				jumpcount++;
				RG.AddForce(transform.up * 15f,ForceMode.Impulse);//押したときに浮かす
